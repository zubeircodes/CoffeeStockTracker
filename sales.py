...
@sales.route('/load_sample_data')
@login_required
def load_sample_data():
    """
    Load sample sales data from a CSV file
    """
    try:
        # Path to sample data file
        sample_data_path = os.path.join('static', 'sample_sales_multi_month.csv')
        
        # Check if the file exists
        if not os.path.exists(sample_data_path):
            flash('Sample data file not found.', 'danger')
            return redirect(url_for('sales.revenue_dashboard'))
        ...
        if not rows:
            flash('No data found in the sample file.', 'warning')
            return redirect(url_for('sales.revenue_dashboard'))
        ...
        db.session.commit()
        flash(f'Successfully loaded {records_added} sample sales records. {records_skipped} records were skipped.', 'success')
